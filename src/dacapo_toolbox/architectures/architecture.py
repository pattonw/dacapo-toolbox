...
class ArchitectureConfig(ABC):
    ...
    def compute_output_shape(self, input_shape: Coordinate) -> Coordinate:
        """
        Method to compute the output shape of the architecture given the input shape.
        """
        module = self.module()
        ...
        dummy_data = torch.zeros((1, self.num_in_channels) + input_shape, device=device)
        with torch.no_grad():
            out = module.forward(dummy_data)
        return Coordinate(out.shape[2:])
