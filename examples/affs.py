...
gt_a_s0 = Array(blobs_a[::2, ::2], offset=(0, 0), voxel_size=(2, 2))
gt_a_s1 = Array(blobs_a[::4, ::4], offset=(0, 0), voxel_size=(4, 4))
gt_b_s0 = Array(blobs_b[::2, ::2], offset=(0, 0), voxel_size=(4, 4))
gt_b_s1 = Array(blobs_b[::4, ::4], offset=(0, 0), voxel_size=(8, 8))
...
for i, batch in enumerate(iter_ds):
    ...
    axs[0, 1].imshow(batch["affs_s0"].permute(1, 2, 0).float())
    axs[0, 2].imshow(batch["affs_mask_s0"].permute(1, 2, 0).float())
    axs[0, 3].imshow(batch["dist_s0"], cmap="gray")
    axs[0, 4].imshow(batch["bmask_s0"], cmap="gray")
    axs[1, 0].imshow(batch["noisy_s1"], cmap="gray")
    axs[1, 1].imshow(batch["affs_s1"].permute(1, 2, 0).float())
    axs[1, 2].imshow(batch["affs_mask_s1"].permute(1, 2, 0).float())
    axs[1, 3].imshow(batch["dist_s1"], cmap="gray")
    axs[1, 4].imshow(batch["bmask_s1"], cmap="gray")

    axs[0, 0].set_title("Raw")
    axs[0, 1].set_title("Affs")
    axs[0, 2].set_title("Affs Mask")
    axs[0, 3].set_title("SDT")
    axs[0, 4].set_title("Boundary Mask")
